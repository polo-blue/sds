...
<PropsTable data={[
  { name: 'items', type: 'DetailItem[]', required: true, description: 'Rows for the table. Each item: { id, label, value, type?, isGenericArray? }.' },
...
interface DetailItem {
  id: string;                   // Unique row id, also used as slot name
  label: string;                // Header label for the row
  value: string | number | boolean | string[] | LinkItem[];
  type?: 'links';               // Use 'links' to render value as LinkItem[]
  isGenericArray?: boolean;     // Render string[] as bulleted list
}
```
